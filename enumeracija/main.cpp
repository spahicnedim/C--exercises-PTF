...
void Dodaj(char* textPitanja, Odgovori tacanOdgovor){
    Pitanje temp;
    strcpy(temp.textPitanja, textPitanja);
...
void prikaziPitanje(int rbPitanja){
    cout << "\n\t" << pitanjaDB[rbPitanja].textPitanja << '\n';
}

int dajBrojTacnih(Student & s) {
// usporediti odgovore studenta iz niza "s.odgovori[]" sa tačnim odgovorima
// preko funkcije "GetTacanOdgovor"
    int brTacnih = 0;
    for (int i = 0; i < brojPitanja; i++)
    {
        if (pitanjaDB[i].odgovorNaPitanje == dajTacanOdgovor(i+1)) {
            brTacnih++;
        }
    };
    return brTacnih;
...
// prikazati sva pitanja preko funkcije "prikaziPitanje"
// sa konzole učitati odgovore i evidentirati ih u niz "student.odgovori[]"
...
void unosPitanja(){
    ...
    cin.getline(pitanjaDB[brojPitanja].textPitanja,100);
    do{
...
void unosPodataka() {
//izvršiti unos podataka za studente
        int x;
        ...
        studentiDB[brojStudenata]._godina = static_cast<godinaStudija>(x);
        cin.ignore();
...
int prikaziStudente() {
    int rb;
    cout << setw(5) << " Rb. " << setw(30) << left << "Ime i prezime" << setw(10) << "Broj indexa\n";
    cout << "-------------------------------------------------------------------\n";
...
void Pretraga(){
    ...
    do
    {
        ...
        if (izbor == 1){
            ...
            for (int i = 0; i < brojStudenata; i++)
            {
                if (studentiDB[i]._godina == static_cast<godinaStudija>(x))
                {
            ...
            if (br==1){
                cout << "Nema studenata na " << vratiGodinu(static_cast<godinaStudija>(x)) << " godini\n";
            }  
            cout << "-------------------------------------------------------------------\n";
        } else if (izbor == 2){
            do
            ...
            if(br==1){
                cout << "Nema " << vratiNacin(static_cast<nacinStudiranja>(x)) << " studenata\n";
            }
            cout << "-------------------------------------------------------------------\n";
        } else if (izbor==3){
            float min, max;
...
int main() {
    ...
    do
    {
        ...
        {
            Student &s = studentiDB[prikaziStudente()];
            if (s.radioTest)    
            {
                cout << "Student " << s.imePrezime << " je radio test\n";
            } else {
                cout << s.imePrezime << " radi test\n";
                pokreniTest(s);
                cout << s.imePrezime << " je zavrsio test\n";
            }
             system("pause"); 
